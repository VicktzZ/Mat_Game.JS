    ...
    // FÁCEIS
        ...
        easy: {
            ...
            q11: async () => {
                ...
                let r = async () => {
                    if (randomArr == 0) await verify(value == randomMax2-randomMin, randomMax2-randomMin)
                    else if (randomArr == 1) await verify(value == Math.floor(randomMax2%randomMin), Math.floor(randomMax2%randomMin))
                    else if (randomArr == 2) await verify(value == Math.floor((randomMax2+randomMin))/2, Math.floor((randomMax2+randomMin)/2))
                    else if (randomArr == 3) await verify(value == Math.floor((randomMax2/2)) + randomMin, Math.floor((randomMax2/2)) + randomMin)
                }
            ...
            q12: async () => {
                ...
                if (rand == 0) await verify(resp.q12 == 50, 50)
                else await verify(resp.q12 == 72, 72)
            },
    ...
    // MÉDIAS
        ...
        medium: {
            ...
            q6: async () => {
                ...
                let percurso1 = random.float(1, 10).toFixed(1)
                let percurso2 = random.float(1, 10).toFixed(1)
                let percurso3 = random.float(1, 10).toFixed(1)

                while (nome == nome2 || nome3 == nome2){
                    nome2 = name()
                }

                while (nome3 == nome || nome2 == nome3){
                    nome3 = name()
                }

                await breakword(`${`Questão ${player.question.count}/30`.blue}\n\nNeste último domingo, ${nome} saiu para andar de bicicleta e decidiu ir até à casa de seu amigo ${nome2}, percorrendo ${`${percurso1} km`.red}.\nDe lá, os dois pedalaram até a casa de ${nome3}, que ficava a uma distância de ${`${percurso2} hm`.red}.\nOs três amigos decidiram ir até o alto da serra da cidade, pedalando mais ${`${percurso3} km`.red}.\nDe casa, até o alto da serra, Caio ${`pedalou quantos metros?`.red}`)

                let lista = [
                    percurso1*1000 + percurso2*100 + percurso3*1000,
                    percurso1*999 + percurso2*100 + percurso3*1000,
                    percurso1*1001 + percurso2*100 + percurso3*1000,
                    percurso1*999 + percurso2*100 + percurso3*1002,
                    percurso1*1000 + percurso2*101 + percurso3*1000
                ]
                ...
                let r = () => {
                    if (lista[0] == '2^5 peças e 2^6 casas') {
                        return `A) ${lista[0]}`
                    } else if (lista[1] == `${percurso1*1000 + percurso2*100 + percurso3*1000}m`) {
                        return `B) ${lista[1]}`
                    } else if (lista[2] == `${percurso1*999 + percurso2*100 + percurso3*1000}m`) {
                        return `C) ${lista[2]}`
                    } else if (lista[3] == `${percurso1*1001 + percurso2*100 + percurso3*1000}m`) {
                        return `D) ${lista[3]}`
                    } else if (lista[4] == `${percurso1*999 + percurso2*100 + percurso3*1002}m`) {
                        return `D) ${lista[4]}`
                    }
                    
                }

                await verify(resp.q1 == `${percurso1*1000 + percurso2*100 + percurso3*1000}m`, r())
            },
